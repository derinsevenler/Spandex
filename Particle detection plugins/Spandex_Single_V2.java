...
//this isn't implemented properly yet
	...
	private boolean showDialog() 
	{
		...
		gd.addCheckbox("Find particles below contrast? (Default determines NP above background brightness)", false);
		...
		if (negContrast)
		{
			//flip parameters because we're now analyzing over a negative range (-1<-0.1)
			thresholdMax = -1*gd.getNextNumber();
			thresholdMin = -1*gd.getNextNumber();
		}
		else
		{
